...
var Channel brewed.Page[*handler.Context] = func(ctx *handler.Context) (brewed.Layout[*handler.Context], templ.Component, error) {
	userID, _ := ctx.UserID()
	props, err := logic.GetChannelPageProps(ctx.Context(), ctx.Database(), ctx.Piped, userID, ctx.Params("id"))
	if err != nil {
		return nil, nil, ctx.Err(err)
	}

	if userID != "" {
		subscribed, err := logic.SubscriptionExists(ctx.Context(), ctx.Database(), ctx.Piped, userID, props.Channel.ID)
		if err != nil {
