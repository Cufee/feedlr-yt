...
func testConnectionToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Connection)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testConnectionToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	connectionDBTypes = map[string]string{`ID`: `TEXT`, `CreatedAt`: `DATE`, `UpdatedAt`: `DATE`, `Type`: `TEXT`, `UserID`: `TEXT`}
	_                 = bytes.MinRead
)
