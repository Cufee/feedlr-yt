...
func testUserOneToOneYoutubeSyncAccountUsingYoutubeSyncAccount(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var foreign YoutubeSyncAccount
	var local User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, youtubeSyncAccountDBTypes, true, youtubeSyncAccountColumnsWithDefault...); err != nil {
	...
	check, err := local.YoutubeSyncAccount().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	AddYoutubeSyncAccountHook(boil.AfterSelectHook, func(ctx context.Context, e boil.ContextExecutor, o *YoutubeSyncAccount) error {
		ranAfterSelectHook = true
		return nil
	})

	slice := UserSlice{&local}
	if err = local.L.LoadYoutubeSyncAccount(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.YoutubeSyncAccount == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.YoutubeSyncAccount = nil
	if err = local.L.LoadYoutubeSyncAccount(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.YoutubeSyncAccount == nil {
		t.Error("struct should have been eager loaded")
...
func testUserOneToOneSetOpYoutubeSyncAccountUsingYoutubeSyncAccount(t *testing.T) {
	...
	var a User
	var b, c YoutubeSyncAccount

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &b, youtubeSyncAccountDBTypes, false, strmangle.SetComplement(youtubeSyncAccountPrimaryKeyColumns, youtubeSyncAccountColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, youtubeSyncAccountDBTypes, false, strmangle.SetComplement(youtubeSyncAccountPrimaryKeyColumns, youtubeSyncAccountColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*YoutubeSyncAccount{&b, &c} {
		err = a.SetYoutubeSyncAccount(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyPlaylists(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.Playlists().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPlaylists(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Playlists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Playlists = nil
	if err = a.L.LoadPlaylists(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Playlists); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManySettings(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, settingDBTypes, false, settingColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, settingDBTypes, false, settingColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.Settings().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadSettings(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Settings); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.Settings = nil
	if err = a.L.LoadSettings(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.Settings); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyViews(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadViews(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpPlaylists(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddPlaylists(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Playlists[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Playlists[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Playlists().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpSettings(t *testing.T) {
	var err error
	...
	foreigners := []*Setting{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, settingDBTypes, false, strmangle.SetComplement(settingPrimaryKeyColumns, settingColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Setting{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddSettings(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.Settings[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.Settings[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.Settings().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpViews(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `TEXT`, `CreatedAt`: `DATE`, `UpdatedAt`: `DATE`, `Permissions`: `TEXT`, `Username`: `TEXT`}
	_           = bytes.MinRead
)
