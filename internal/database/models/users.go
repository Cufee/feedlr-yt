...
var UserWhere = struct {
	...
	Permissions: whereHelperstring{field: "\"users\".\"permissions\""},
...
var UserRels = struct {
	...
	YoutubeTVSyncAccount: "YoutubeTVSyncAccount",
	Playlists:            "Playlists",
	Settings:             "Settings",
...
type userR struct {
	...
	YoutubeTVSyncAccount *YoutubeTVSyncAccount `boil:"YoutubeTVSyncAccount" json:"YoutubeTVSyncAccount" toml:"YoutubeTVSyncAccount" yaml:"YoutubeTVSyncAccount"`
...
func (o *User) GetYoutubeTVSyncAccount() *YoutubeTVSyncAccount {
	if o == nil {
		return nil
	}

	return o.R.GetYoutubeTVSyncAccount()
}

func (r *userR) GetYoutubeTVSyncAccount() *YoutubeTVSyncAccount {
	if r == nil {
		return nil
	}

	return r.YoutubeTVSyncAccount
}
...
func (o *User) GetPlaylists() PlaylistSlice {
	...
	return o.R.GetPlaylists()
}

func (r *userR) GetPlaylists() PlaylistSlice {
	if r == nil {
		return nil
	}
...
func (o *User) GetSettings() SettingSlice {
	if o == nil {
		return nil
	}

	return o.R.GetSettings()
}

func (r *userR) GetSettings() SettingSlice {
	if r == nil {
		return nil
	}
...
func (o *User) GetViews() ViewSlice {
	if o == nil {
		return nil
	}
...
func (r *userR) GetViews() ViewSlice {
	if r == nil {
		return nil
	}
...
var (
	userAllColumns            = []string{"id", "created_at", "updated_at", "permissions", "username"}
	userColumnsWithoutDefault = []string{"id", "created_at", "updated_at", "username"}
	userColumnsWithDefault    = []string{"permissions"}
...
// YoutubeTVSyncAccount pointed to by the foreign key.
func (o *User) YoutubeTVSyncAccount(mods ...qm.QueryMod) youtubeTVSyncAccountQuery {
	queryMods := []qm.QueryMod{
		qm.Where("\"user_id\" = ?", o.ID),
	}
	...
	return YoutubeTVSyncAccounts(queryMods...)
}
...
// Settings retrieves all the setting's Settings with an executor.
func (o *User) Settings(mods ...qm.QueryMod) settingQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"settings\".\"user_id\"=?", o.ID),
	)
...
func (o *User) Views(mods ...qm.QueryMod) viewQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
...
func (userL) LoadYoutubeSyncAccount(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
...
// LoadYoutubeTVSyncAccount allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (userL) LoadYoutubeTVSyncAccount(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`youtube_tv_sync_accounts`),
		qm.WhereIn(`youtube_tv_sync_accounts.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load YoutubeTVSyncAccount")
	}

	var resultSlice []*YoutubeTVSyncAccount
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice YoutubeTVSyncAccount")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for youtube_tv_sync_accounts")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for youtube_tv_sync_accounts")
	}
	...
	if singular {
		foreign := resultSlice[0]
		object.R.YoutubeTVSyncAccount = foreign
		if foreign.R == nil {
			foreign.R = &youtubeTVSyncAccountR{}
		}
		foreign.R.User = object
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.ID == foreign.UserID {
				local.R.YoutubeTVSyncAccount = foreign
				if foreign.R == nil {
					foreign.R = &youtubeTVSyncAccountR{}
				}
				foreign.R.User = local
...
func (userL) LoadPlaylists(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`playlists`),
		qm.WhereIn(`playlists.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadSettings allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadSettings(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`settings`),
		qm.WhereIn(`settings.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load settings")
	}

	var resultSlice []*Setting
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice settings")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on settings")
	}
	if err = results.Err(); err != nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Settings = append(local.R.Settings, foreign)
				if foreign.R == nil {
					foreign.R = &settingR{}
				}
				foreign.R.User = local
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadViews(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`views`),
		qm.WhereIn(`views.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// SetYoutubeTVSyncAccount of the user to the related item.
// Sets o.R.YoutubeTVSyncAccount to related.
// Adds o to related.R.User.
func (o *User) SetYoutubeTVSyncAccount(ctx context.Context, exec boil.ContextExecutor, insert bool, related *YoutubeTVSyncAccount) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			YoutubeTVSyncAccount: related,
		}
	} else {
		o.R.YoutubeTVSyncAccount = related
	}

	if related.R == nil {
		related.R = &youtubeTVSyncAccountR{
			User: o,
		}
	} else {
		related.R.User = o
...
func (o *User) AddPlaylists(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Playlist) error {
	var err error
...
// AddSettings adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Settings.
// Sets related.R.User appropriately.
func (o *User) AddSettings(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Setting) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
	...
	if o.R == nil {
		...
		o.R.Settings = append(o.R.Settings, related...)
	}
...
// Sets related.R.User appropriately.
func (o *User) AddViews(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*View) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
