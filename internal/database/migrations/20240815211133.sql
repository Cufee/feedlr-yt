...
CREATE TABLE `new_views` (`id` text NOT NULL, `created_at` date NOT NULL, `updated_at` date NOT NULL, `user_id` text NOT NULL, `video_id` text NOT NULL, `progress` integer NOT NULL, `hidden` boolean NOT NULL DEFAULT false, PRIMARY KEY (`id`), CONSTRAINT `views_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE, CONSTRAINT `views_video_id_fkey` FOREIGN KEY (`video_id`) REFERENCES `videos` (`id`));
-- Copy rows from old table "views" to new temporary table "new_views"
INSERT INTO `new_views` (`id`, `created_at`, `updated_at`, `user_id`, `video_id`, `progress`) SELECT `id`, `created_at`, `updated_at`, `user_id`, `video_id`, `progress` FROM `views`;
