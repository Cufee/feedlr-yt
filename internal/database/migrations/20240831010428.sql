...
CREATE TABLE `new_videos` (`id` text NOT NULL, `created_at` date NOT NULL, `updated_at` date NOT NULL, `title` text NOT NULL, `description` text NOT NULL, `duration` integer NOT NULL, `published_at` date NOT NULL, `private` boolean NOT NULL, `type` text NOT NULL, `thumbnail` text NOT NULL DEFAULT '', `channel_id` text NOT NULL, PRIMARY KEY (`id`), CONSTRAINT `videos_channel_id_fkey` FOREIGN KEY (`channel_id`) REFERENCES `channels` (`id`) ON DELETE CASCADE);
